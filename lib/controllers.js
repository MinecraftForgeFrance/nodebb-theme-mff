...
Controllers.renderThemeSettings = async (req, res, next) => {
	...
	const mffSkin = await user.getUserField(req.uid, "mffthemeskin");
    userData.darkTheme = mffSkin === "dark";
...
Controllers.updateThemeSettings = async (req, res) => {
	await user.setUserField(req.uid, "mffthemeskin", req.body.mffThemeSkin);
